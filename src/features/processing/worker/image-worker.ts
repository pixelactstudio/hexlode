  ...
  try {
    ...
    if (output.type !== 'image/webp') {
      post({
        ...
        id: request.id,
        code: 'encode_failed',
        message: 'This browser could not create a WebP image.',
      })
      return
    }

    const outputBuffer = await output.arrayBuffer()
    const warnings = ['Embedded metadata is not retained in the WebP output.']
    if (output.size >= request.input.buffer.byteLength) {
      warnings.push('The WebP output is not smaller than the source image.')
    }
